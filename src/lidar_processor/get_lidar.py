...
def broadcast_tf(event):
	tf_broadcaster.sendTransform((meters(-10)+0.04044,meters(-5)-0.01426,0),tf.transformations.quaternion_from_euler(0,0,pi/2),rospy.Time.now(),'base_lidar','base_link')
	tf_broadcaster.sendTransform((meters(6.875),0,0),tf.transformations.quaternion_from_euler(0,0,line_angle),rospy.Time.now(),'base_camera','base_link')
